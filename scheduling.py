...
def main():
	...
	for j in range(Days):
		purchase=0
		purchase_dict={}#{servers_type:num,...}
		migration=0
		migration_list=[]#[[vm_id,servers_id,node],...]
				...
				if request[0]=='add':#部署
					if vm_Dict[request[1]][2]==0:#单节点部署
						...
						for i in range(len(servers_Pool)):#这个循环可以跟上面那个合并
							...
							if servers_Pool[i][1]>vm_need[0] and servers_Pool[i][3]>vm_need[0] and servers_Pool[i][2]>vm_need[1] and servers_Pool[i][4]>vm_need[1]:
					...
					servers_Pool[servers_id][4]+=vm_Pool[request[1]][4]#释放资源
					process=1#需求已处理
		...
		sys.stdout.write('(purchase,'+str(purchase)+')\n')#输出,后期写成一个函数
		for key,values in purchase_dict.items():
			sys.stdout.write('('+str(key)+','+str(values)+')\n')
		sys.stdout.write('(migration,'+str(migration)+')\n')
		...
		for sch in scheme:
			if len(sch)==1:
				sys.stdout.write('('+str(sch[0])+')\n')
			if len(sch)==2:
				sys.stdout.write('('+str(sch[0])+','+str(sch[1])+')\n')

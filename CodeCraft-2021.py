...
def process_requests(requests):
	for request in requests:
		if request[0] == 'add':  # 部署
			vm_model = request[1]
			is_enough = False  # 标记是否放的下，放不下就要买新的服务器了
			is_type = None
			if vm_model in vm_L:
				is_type = 'L'
				for i in range(len(servers_Pool)):
					if servers_Pool.servers_pool[i][6] == 'L':
						is_enough = servers_Pool.Add_vm(i, [*vm_Dict[vm_model][0:2], *vm_Dict[vm_model][0:2]])
						if is_enough == False:
							continue
						else:
							vm_Pool[request[2]] = [i, *vm_Dict[vm_model][0:2], *vm_Dict[vm_model][0:2]]
							break
			elif vm_model in vm_M:
				is_type = 'M'
				for i in range(len(servers_Pool)):
					if servers_Pool.servers_pool[i][6] == 'M':
						is_enough = servers_Pool.Add_vm(i, [*vm_Dict[vm_model][0:2], *vm_Dict[vm_model][0:2]])
						if is_enough == False:
							continue
						else:
				...
				is_type = 'S'
				for i in range(len(servers_Pool)):
					if servers_Pool.servers_pool[i][6] == 'S':
						is_enough = servers_Pool.Add_vm(i, [*vm_Dict[vm_model][0:2], *vm_Dict[vm_model][0:2]])
						if is_enough == False:
							continue
						else:
			...
			if is_enough == False:  # 需要购买新的服务器
				...
				# 买好了再把虚拟机放进去
				servers_Pool.Add_vm(len(servers_Pool) - 1, [*vm_Dict[vm_model][0:2], *vm_Dict[vm_model][0:2]])
